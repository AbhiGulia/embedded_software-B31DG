...
void freq1(){
  ...
  //Serial.println(last_freq_signal_1);
  monitor.jobEnded(2);
}
...
void freq2(){
  ...
  //Serial.println(last_freq_signal_2);
  monitor.jobEnded(3);
}
...
uint16_t freqCounter(uint8_t read_pin, uint16_t minfreq, uint16_t maxfreq){
  ...
  while(digitalRead(read_pin) == pin_status && micros() - timeout_counter < max_timeperiod); // wait for the signal to change state with a timeout period
  uint16_t freq = 1000000/(2*(micros() - freq_start_time));                                      // freq = 1/timeperiod changed in secs from micros and divided by 2 as we are only measuring either high or low time
  if(freq < minfreq + 0.025*maxfreq || freq > maxfreq + 0.025*maxfreq){                      // checking range of frequency with 2.5% error margin
    //Serial.println(" WARNING! frequency of signal is out of range. ");
  }
  return freq;
}
